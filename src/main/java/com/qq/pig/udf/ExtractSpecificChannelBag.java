 ...
 * Time: 下午4:49
 */
public class ExtractSpecificChannelBag extends EvalFunc<DataBag>
{

    private Splitter commaSplitter = Splitter.on(",");
    private static Set<String> channels = Sets.newHashSet();
    ...
    static
    {
        ...
        channels.add("milhqj");
        channels.add("militp");
        channels.add("2011society");
        channels.add("phsoci");
        channels.add("society");
    }
    ...
    public DataBag exec(Tuple input) throws IOException
    {
        ...
        try
        {
            ...
            if (channelInterestStr == null)
                return null;
            Iterable<String> iterable = commaSplitter.split(channelInterestStr);
            for (String channelInterest : iterable)
            {
                if (!channelInterest.startsWith("news:"))
                    continue;

                Iterable<String> iter = Splitter.on(":").split(channelInterest);
