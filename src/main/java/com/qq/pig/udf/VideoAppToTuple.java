 ...
 */
public class VideoAppToTuple extends EvalFunc<Tuple>
{
    ...
    public static List<Object> toList(String line) throws IOException
    {
        ...
        String ui = (String) jsonStr.get(UI);
        result.add(ui);
        String vod_live_cdnId = null;
        String vod_live_playerId = null;
        Map<?, ?> kv = (Map<?, ?>) jsonStr.get(KV);
        if (kv != null)
        {
            vod_live_cdnId = (String) kv.get(KV_VOID_LIVE_CDNID);
            vod_live_playerId = (String) kv.get(KV_VOD_LIVE_PLAYERID);
        }
        result.add(vod_live_cdnId);
        result.add(vod_live_playerId);
        String ei = (String) jsonStr.get(EI);
        result.add(ei);
        Long et = Long.valueOf(jsonStr.get(ET).toString());
        result.add(et);
        Long id = Long.valueOf(jsonStr.get(ID).toString());
        result.add(id);
...
//    @Override
    ...
    public Schema outputSchema(Schema input)
    {
        ...
        tupleSchema.add(new Schema.FieldSchema("ui", DataType.CHARARRAY));
        tupleSchema.add(new Schema.FieldSchema("vod_live_cdnId", DataType.CHARARRAY));
        tupleSchema.add(new Schema.FieldSchema("vod_live_playerId", DataType.CHARARRAY));
